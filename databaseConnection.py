...
def persistBitfinex(jsonBitfinex):
    ...
    with Session() as session:
        for element in jsonBitfinex:
            bitelement=bitfin_ticker(element[0][1:],element[1],element[2],element[3],element[4],element[5],element[6],element[7],element[8],element[9],element[10],setOfRequestObtained)
    ...
    with Session() as session:
        for element in jsonCexio['data']:
            ...
            if symbol in arrayOfCurrencies:                    
                cexioElement=cexio_ticker(symbol,element['low'],element['high'],element['last'],element['volume'],element['volume30d'],element['priceChange'],element['priceChangePercentage'],element['bid'],element['ask'],setOfRequestObtained)
